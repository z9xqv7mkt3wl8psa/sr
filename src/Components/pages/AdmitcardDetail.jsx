...
const AdmitcardDetail = () => {
  const { id } = useParams();
  const admitcard = admitcardData.find((item) => item.id === id);

  if (!admitcard) return <h2 className="text-center text-red-500 mt-10">❌ Page Not Found</h2>;
    ...
    <div className="max-w-5xl mx-auto bg-white shadow-md border p-6 mt-6 text-sm leading-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 border mt-6">
        <div className="border p-3">
          ...
          <ul className="mt-2 space-y-1">
            {admitcard.importantDates.map((item, i) => (
        ...
        <div className="border p-3">
          ...
          <ul className="mt-2 space-y-1">
            {admitcard.applicationFees.map((item, i) => (
              <li key={i} className={getHighlightClass(item.highlight)}>
                <b>{item.label}:</b> {item.value}
              </li>
            ))}
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {admitcard.qualifications.map((q, i) => (
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {admitcard.requiredDocs.map((doc, i) => <li key={i}>{doc}</li>)}
        </ul>
      </div>
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {admitcard.selectionProcess.map((step, i) => <li key={i}>{step}</li>)}
        </ul>
      </div>
      ...
      <div className="border p-3 mt-6">
        ...
        <ol className="list-decimal pl-6 mt-2 space-y-1">
          {admitcard.howToApply.map((step, i) => <li key={i}>{step}</li>)}
        </ol>
      </div>
