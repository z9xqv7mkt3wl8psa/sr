...
import latestjobsData from "../../data/latestjobsData";


const LatestjobsDetail = () => {
  const { id } = useParams();
  const latestjobs = latestjobsData.find((item) => item.id === id);

  if (!latestjobs) return <h2 className="text-center text-red-500 mt-10">❌ Page Not Found</h2>;
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white shadow-md border p-6 mt-6 text-sm leading-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 border mt-6">
        <div className="border p-3">
          ...
          <ul className="mt-2 space-y-1">
            {latestjobs.importantDates.map((item, i) => (
        ...
        <div className="border p-3">
          ...
          <ul className="mt-2 space-y-1">
            {latestjobs.applicationFees.map((item, i) => (
              <li key={i} className={getHighlightClass(item.highlight)}>
                <b>{item.label}:</b> {item.value}
              </li>
            ))}
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="mt-2 space-y-1">
          {latestjobs.ageLimit.map((item, i) => (
            <li key={i} className={getHighlightClass(item.highlight)}>
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {latestjobs.qualifications.map((q, i) => (
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {latestjobs.requiredDocs.map((doc, i) => <li key={i}>{doc}</li>)}
        </ul>
      </div>
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {latestjobs.selectionProcess.map((step, i) => <li key={i}>{step}</li>)}
        </ul>
      </div>
      ...
      <div className="border p-3 mt-6">
        ...
        <ol className="list-decimal pl-6 mt-2 space-y-1">
          {latestjobs.howToApply.map((step, i) => <li key={i}>{step}</li>)}
        </ol>
      </div>
...
<div className="border p-3 mt-6">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
    {latestjobs.linksBox.map((link, i) => (
      <div
        key={i}
...
export default LatestjobsDetail;
