...
const HeadlineDetail = () => {
  const { id } = useParams();
  const headline = headlineData.find((item) => item.id === id);

  if (!headline) return <h2 className="text-center text-red-500 mt-10">❌ Page Not Found</h2>;
    ...
    <div className="max-w-5xl mx-auto bg-white shadow-md border p-6 mt-6 text-sm leading-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 border mt-6">
        <div className="border p-3">
          ...
          <ul className="mt-2 space-y-1">
            {headline.importantDates.map((item, i) => (
        ...
        <div className="border p-3">
          ...
          <ul className="mt-2 space-y-1">
            {headline.applicationFees.map((item, i) => (
              <li key={i} className={getHighlightClass(item.highlight)}>
                <b>{item.label}:</b> {item.value}
              </li>
            ))}
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {headline.qualifications.map((q, i) => (
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {headline.requiredDocs.map((doc, i) => <li key={i}>{doc}</li>)}
        </ul>
      </div>
      ...
      <div className="border p-3 mt-6">
        ...
        <ul className="list-disc pl-6 mt-2 space-y-1">
          {headline.selectionProcess.map((step, i) => <li key={i}>{step}</li>)}
        </ul>
      </div>
      ...
      <div className="border p-3 mt-6">
        ...
        <ol className="list-decimal pl-6 mt-2 space-y-1">
          {headline.howToApply.map((step, i) => <li key={i}>{step}</li>)}
        </ol>
      </div>

      {/* Colorful Sarkari Style Boxes */}
      <div className="border p-3 mt-6">
        <h3 className="font-bold text-purple-700 underline mb-3">Some Useful Headline Links</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {headline.linksBox.map((link, i) => (
            <div key={i} className="flex justify-between items-center border p-2 bg-gray-50">
